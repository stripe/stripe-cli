...
func TestWorkbenchClientListAndRetrieve(t *testing.T) {
	...
	require.Len(t, blueprint.BlueprintParams, 1)
	assert.Equal(t, "amount", blueprint.BlueprintParams[0].Params[0].Name)
	assert.Equal(t, "${blueprint_settings.payment:country}", blueprint.StepRefs[0].Settings["country"])
	assert.Equal(t, "${blueprint_params.payment:amount}", blueprint.StepRefs[0].Params["amount"])
	require.Len(t, blueprint.Steps, 1)
	assert.Equal(t, "${blueprint_settings.payment:country}", blueprint.Steps[0].Settings["country"])
