...
func DeployGuidedAction() *coop.GuidedAction {
	return &coop.GuidedAction{
		...
		Steps: []coop.SessionStep{
			{
				WorkbenchStepDefinition: step("detect-deploy-path", "Detect deploy path"),
			...
			{
				WorkbenchStepDefinition: step("configure-deploy", "Configure deployment"),
				Nodes: []coop.SessionNode{
			...
			{
				WorkbenchStepDefinition: step("deploy", "Deploy"),
				Nodes: []coop.SessionNode{
			...
			{
				WorkbenchStepDefinition: step("verify-deployment", "Verify deployment"),
				Nodes: []coop.SessionNode{
...
func DeployUpdateGuidedAction(target string) *coop.GuidedAction {
	...
	return &coop.GuidedAction{
		...
		Steps: []coop.SessionStep{
			{
				WorkbenchStepDefinition: step("confirm-target", "Confirm deployment target"),
				Nodes: []coop.SessionNode{
			...
			{
				WorkbenchStepDefinition: step("push-deploy", "Push deployment"),
				Nodes: []coop.SessionNode{
...
func step(key, title string) coop.WorkbenchStepDefinition {
	return coop.WorkbenchStepDefinition{
		Key:   key,
...
func node(nodeType coop.NodeType, key, title, description, reviewPrompt string) coop.SessionNode {
	return coop.SessionNode{
		WorkbenchBlueprintNode: coop.WorkbenchBlueprintNode{
			...
			Key:         key,
			Title:       coop.MessageDescriptor{DefaultMessage: title},
			Description: coop.MessageDescriptor{DefaultMessage: description},
		},
