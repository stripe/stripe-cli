...
func TestUpdateKeyConfirmStepReviewRequiresStepSelection(t *testing.T) {
	dir := t.TempDir()
	store, _ := coop.NewStoreAt(dir)
	...
	node1, _ := updated.session.NodeByNumber(1)
	node2, _ := updated.session.NodeByNumber(2)
	assert.Equal(t, coop.NodeReview, node1.State)
	...
	newSession := &coop.Session{
		...
		Steps: []coop.SessionStep{
			{Key: "ch1", Title: "Ch", Nodes: []coop.SessionNode{
				{Key: "n1", Title: "Step", State: coop.NodeActive},
			}},
		},
	}
...
func TestCompletionTransitionResetsCursor(t *testing.T) {
	...
	newSession := &coop.Session{
		...
		Steps: []coop.SessionStep{
			{Key: "ch1", Title: "Ch", Nodes: []coop.SessionNode{
				{Key: "n1", Title: "Step 1", State: coop.NodeDone},
				{Key: "n2", Title: "Step 2", State: coop.NodeDone},
				{Key: "n3", Title: "Step 3", State: coop.NodeDone},
			}},
		},
	}
