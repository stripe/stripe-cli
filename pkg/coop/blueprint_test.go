...
func TestNewSessionRejectsInvalidNodeReferences(t *testing.T) {
	tests := []struct {
		name      string
		reference string
		contains  string
	}{
		{name: "self", reference: "${node.create-payment:id}", contains: "before it has completed"},
		{name: "forward", reference: "${node.handle-payment:id}", contains: "before it has completed"},
		{name: "malformed", reference: "${node.create-payment:id", contains: "malformed node reference"},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			blueprint := loadTestBlueprint(t)
			blueprint.Steps[0].Nodes[0].APIRequestDetails.Fixture.Path = tt.reference

			_, err := NewSessionFromBlueprint(blueprint, "invalid_reference", nil, nil)
			require.Error(t, err)
