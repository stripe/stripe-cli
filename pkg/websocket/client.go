...
var ErrUnknownID = errorcategory.New(errorcategory.API, unknownIDMessage)
...
func wsDialThroughProxy(ctx context.Context, proxyURL *url.URL, addr string) (net.Conn, error) {
	...
	if resp.StatusCode != http.StatusOK {
		conn.Close()
		return nil, errorcategory.Errorf(errorcategory.API, "proxy CONNECT %s: %s", addr, resp.Status)
	}
	return conn, nil
}
