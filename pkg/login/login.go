...
// code has actually expired, the server returned a terminal OAuth error (e.g. access_denied),
// or the token exchange succeeded but saving the resulting credentials failed (the device code
// is already consumed at that point, so it can't be retried); on success, also clears the
// pending state.
func CheckPendingOAuthLogin(ctx context.Context, cfg *config.Config) (*DeviceCodeLoginResult, error) {
	...
	if err != nil {
		// The device code has already been redeemed for tokenResp, so it can't be used again -
		// retrying with the same pending state would just fail with a confusing OAuth error
		// instead of surfacing this one. Matches PollPendingOAuthLogin's behavior.
		clearPendingDeviceAuth()
		return nil, err
	}
	clearPendingDeviceAuth()
