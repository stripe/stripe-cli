...
func main() {
	...
	// Load the wiring template with a custom function map
...
func generateSpecFiles(data *TemplateData, outputDir string) error {
		...
		for nsName, nsDataInner := range nsData.Namespaces {
			for _, resData := range nsDataInner.Resources {
				for _, opData := range resData.Operations {
					...
					groups[key] = append(groups[key], varEntry{opData.VarName, opData})
				}
	...
	for key, entries := range groups {
		content, err := generateSpecFileContent(entries)
		if err != nil {
...
// generateSpecFileContent produces the []byte for a specs_*_gen.go file.
func generateSpecFileContent(entries []varEntry) ([]byte, error) {
	var sb strings.Builder

	sb.WriteString("// Code generated by gen_resources_cmds.go; DO NOT EDIT.\n\n")
	sb.WriteString("package resources\n\n")
	sb.WriteString("import \"github.com/stripe/stripe-cli/pkg/cmd/resource\"\n\n")

	for _, entry := range entries {
		op := entry.opData
		sb.WriteString(fmt.Sprintf("var %s = resource.OperationSpec{\n", entry.varName))
		sb.WriteString(fmt.Sprintf("\tName:      %q,\n", op.Name))
		sb.WriteString(fmt.Sprintf("\tPath:      %q,\n", op.Path))
		sb.WriteString(fmt.Sprintf("\tMethod:    %q,\n", strings.ToUpper(op.HTTPVerb)))
		if op.IsPreview {
			sb.WriteString("\tIsPreview: true,\n")
		}
		if op.ServerURL != "" {
			sb.WriteString(fmt.Sprintf("\tServerURL: %q,\n", op.ServerURL))
		}
		if op.Summary != "" {
		...
		if len(op.Params) > 0 {
			sb.WriteString("\tParams: map[string]*resource.ParamSpec{\n")
			for paramName, ps := range op.Params {
				...
				if len(ps.Enum) > 0 {
					sb.WriteString("\t\t\tEnum: []resource.EnumSpec{\n")
					for _, ev := range ps.Enum {
						sb.WriteString(fmt.Sprintf("\t\t\t\t{Value: %q", ev.Value))
						if ev.Description != "" {
							sb.WriteString(fmt.Sprintf(", Description: %q", ev.Description))
						}
						sb.WriteString("},\n")
					}
					sb.WriteString("\t\t\t},\n")
