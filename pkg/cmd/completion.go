...
func newCompletionCmd() *completionCmd {
	...
	cc.cmd.Flags().StringVar(&cc.shell, "shell", "",
		"The shell to generate completion commands for. Supports \"bash\", \"zsh\", or \"fish\"")
	cc.cmd.Flags().BoolVar(&cc.writeToStdout, "write-to-stdout", false,
		"Print completion script to stdout rather than creating a new file.")
...
1. Move the completion script to your Fish completions directory:
    mkdir -p ~/.config/fish/completions
    mv stripe-completion.fish ~/.config/fish/completions/

2. If you already have Fish running, reload your current shell or open a new one. Fish automatically loads any completion scripts found in ~/.config/fish/completions/ at startup.

3. (Optional) Verify by typing 'stripe ' then pressing TAB to see completion suggestions.`
)
...
func selectShell(shell string, writeToStdout bool) error {
	...
	default:
		return fmt.Errorf("Could not automatically detect your shell. Please run the command with the `--shell` flag for either bash, zsh, or fish")
	}
}
...
func genFish(writeToStdout bool) error {
	...
	err := rootCmd.GenFishCompletionFile("stripe-completion.fish", true)
	if err == nil {
