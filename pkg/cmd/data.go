...
namespace (e.g. revenue.mrr and revenue.arr both belong to the "revenue"
...
See https://docs.stripe.com/data/analytics for supported metrics.`,
  ...
  stripe data metrics run \
	...
	c.cmd.Flags().StringArrayVar(&c.metrics, "metric", []string{}, "Metric to query: common name (e.g. revenue.mrr) or ID (e.g. metric_61Sud3n5oAGVCWiSr5). Repeatable; all metrics must share the same namespace. [required]")
	c.cmd.Flags().StringVar(&c.startsAt, "starts-at", "", "Start of the time range in RFC3339 format (e.g. 2026-01-01T00:00:00Z) [required]")
	c.cmd.Flags().StringVar(&c.endsAt, "ends-at", "", "End of the time range in RFC3339 format (e.g. 2026-01-31T23:59:59Z) [required]")
	c.cmd.Flags().StringVar(&c.granularity, "granularity", "day", "Time granularity: day, week, month, or year")
	c.cmd.Flags().StringArrayVar(&c.groupBy, "group-by", []string{}, "Dimension to group results by (e.g. price, product, customer). At most one allowed.")
...
func (c *dataMetricsRunCmd) runDataMetricsRunCmd(cmd *cobra.Command, args []string) error {
	...
	// Forward all parameters to the API and let it validate them (metric
	// namespaces, time ranges, group-by/filter cardinality, limit bounds, etc.),
	// rather than duplicating those rules here where they could drift out of sync.
