...
func TestRunOperationCmd(t *testing.T) {
	ts := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		require.Equal(t, "value1", vals["param1"][0])
		require.Equal(t, "value2", vals["param2"][0])
		require.Equal(t, "some_value", vals["param_with_underscores"][0])
		require.Equal(t, "some_other_value", vals["param[with][dots]"][0])
		require.Equal(t, []string{"data1", "data2"}, vals["param_array[]"])
	}))
	defer ts.Close()
...
func TestRunOperationCmd_ExtraParams(t *testing.T) {
	ts := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		require.NoError(t, err)
		require.Equal(t, "value1", vals["param1"][0])
		require.Equal(t, "123 Main St", vals["shipping[address][line1]"][0])
		require.Equal(t, "name", vals["shipping[name]"][0])
	}))
	defer ts.Close()
