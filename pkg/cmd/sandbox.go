...
type sandboxDeleteClient interface {
	PrepareDelete(context.Context, string) (*sandbox.DeleteTarget, error)
	Delete(context.Context, *sandbox.DeleteTarget) error
}
...
Pass the sandbox ACCOUNT (acct_...) shown by ` + "`stripe sandbox list`" + `. The
command always asks for confirmation and cannot run non-interactively.`,
...
func (sdc *sandboxDeleteCmd) runSandboxDeleteCmd(cmd *cobra.Command, args []string) error {
	...
	if len(accountID) <= len("acct_") || !strings.HasPrefix(accountID, "acct_") {
		return errorcategory.New(errorcategory.UserInput, "a valid sandbox account id (acct_...) is required; run `stripe sandbox list` to find one")
	}
	if !sandboxDeleteInteractive(cmd) {
		return errorcategory.New(errorcategory.UserInput, "sandbox deletion requires an interactive terminal")
	}
	...
	target, err := client.PrepareDelete(cmd.Context(), accountID)
	if err != nil {
		return err
	}

	out := cmd.OutOrStdout()
	fmt.Fprintf(out, "Delete sandbox %q (%s)? This cannot be undone. [y/N] ", target.Name, target.AccountID)
	...
	if answer != "y" && answer != "yes" {
		fmt.Fprintln(out, "Aborted. No changes were made.")
		return nil
	}

	if err := client.Delete(cmd.Context(), target); err != nil {
		return err
	}
	fmt.Fprintf(out, "Deleted sandbox %q (%s)\n", target.Name, target.AccountID)
	return nil
}

func sandboxDeleteInteractive(cmd *cobra.Command) bool {
