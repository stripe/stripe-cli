...
Only await human review when the next command says to. Before awaiting, run the supplied review command, keep useful servers running, and give the developer concrete actions and expected results. When a response sets "advance_allowed" to false, the developer has not acted yet: run the command in "next" again immediately, as many times as it takes. Any other response — including one with no "advance_allowed" — is followed normally. Co-op returns from await-review on its own after about %s, so do not set a shell timeout below %s and do not run it in the background. A "waiting" state is normal, not an error — never treat it as confirmation, never skip ahead, and never stop to ask a question while a step is in review, because the developer is watching the TUI, not your pane. If a command fails, its "recovery" object holds the hint and the command to run next. If you ever lose track of the lifecycle, run "stripe coop agent resume --session=<session_id>" using the session_id from any Co-op response, to read the current state and its exact next command. If changes are requested, redo the affected task from the feedback. After the final confirmation, immediately run the returned next command.
