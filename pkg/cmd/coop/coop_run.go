...
Only await human review when the next command says to. Before awaiting, run the supplied review command, keep useful servers running, and give the developer concrete actions and expected results. Co-op returns from await-review after %s; allow the shell command at least %s so the structured timeout response arrives. Re-run it if Co-op reports a timeout. If you ever lose track of the lifecycle, run "stripe coop agent resume" to read the current state and its exact next command. If changes are requested, redo the affected task from the feedback. After the final confirmation, immediately run the returned next command.
