...
Only await human review when the next command says to. Before awaiting, run the supplied review command, keep useful servers running, and give the developer concrete actions and expected results. Every response carries "advance_allowed": when it is false the developer has not acted yet, so run the command in "next" again immediately, as many times as it takes; when it is true, follow "next". Co-op returns from await-review on its own after about %s, so do not set a shell timeout below %s and do not run it in the background. A "waiting" state is normal, not an error — never treat it as confirmation, never skip ahead, and never stop to ask a question while a step is in review, because the developer is watching the TUI, not your pane. If you ever lose track of the lifecycle, run "stripe coop agent resume" to read the current state and its exact next command. If changes are requested, redo the affected task from the feedback. After the final confirmation, immediately run the returned next command.
