...
func newCoopAgentReportWorkCmd() *coopAgentActionCmd {
	...
	c.cmd = &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			if c.stdin {
				if err := c.readWorkFromStdin(); err != nil {
					return outputAgentResponse(coop.CommandResponse{OK: false, Error: err.Error(), Hint: "Pipe valid JSON to --stdin"}, nil)
				}
			}
			service, err := newWorkflowService()
			if err != nil {
	...
	c.cmd.Flags().BoolVar(&c.stdin, "stdin", false, "Read implementation data as JSON from stdin")
	return c
}
