...
func init() {
	...
	err := postProcessResourceCommands(rootCmd, &Config)
	if err != nil {
		log.Fatal(err)
	}
...
func postProcessResourceCommands(rootCmd *cobra.Command, cfg *config.Config) error {
	err := resource.AddEventsSubCmds(rootCmd, cfg)
	if err != nil {
		return err
	}

	err = resource.AddTerminalSubCmds(rootCmd, cfg)
	if err != nil {
		return err
	}

	err = resource.AddOrdersSubCmds(rootCmd, cfg)
	if err != nil {
		return err
	}

	// remove autogenerated apps command
	err = resource.RemoveAppsCmd(rootCmd)
	if err != nil {
		return err
	}
