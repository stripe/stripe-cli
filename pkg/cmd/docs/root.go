...
func (r *RootCommand) run(cmd *cobra.Command, args []string) error {
	...
	if r.client == nil {
		return errorcategory.Errorf(errorcategory.UserInput, "docs client not initialized")
	}
	if r.renderer == nil {
		return errorcategory.Errorf(errorcategory.UserInput, "markdown renderer not initialized")
	}
