...
func Browser(url string) error {
	...
	default:
		err = errorcategory.Errorf(errorcategory.Internal, "unsupported platform")
	}
...
func OpenURL(ctx context.Context, u *url.URL, allowedHosts map[string]bool) error {
	if u == nil {
		return errorcategory.Errorf(errorcategory.Internal, "nil URL")
	}
	if u.Scheme != "https" || (allowedHosts != nil && !allowedHosts[u.Host]) {
		return errorcategory.Errorf(errorcategory.UserInput, "URL not allowed: %s", u)
	}
...
func openCommand() ([]string, error) {
	...
	default:
		return nil, errorcategory.Errorf(errorcategory.Internal, "unsupported platform: %s", runtime.GOOS)
	}
}
