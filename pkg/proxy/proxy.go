	...
	for nAttempts < maxConnectAttempts {
		...
		if err != nil {
			p.cfg.OutCh <- websocket.ErrorElement{
				Error: errorcategory.Errorf(errorcategory.API, "Error while authenticating with Stripe: %v", err),
			}
			return err
		}
		...
		*p.cfg.DeviceToken = session.DeviceToken
		...
		case <-p.webSocketClient.NotifyExpired:
			if nAttempts < maxConnectAttempts {
				...
				err := errorcategory.Errorf(errorcategory.API, "session expired, terminating after %d failed attempts to reauthorize", nAttempts)
				p.cfg.OutCh <- websocket.ErrorElement{
					Error: err,
				}
				return err
		...
		for i := 0; i <= 5; i++ {
			...
			if clientError, ok := stripeauth.IsAuthorizationClientError(err); ok {
				if clientError.StatusCode == http.StatusTooManyRequests {
					err = errorcategory.New(errorcategory.API, "you have too many `stripe listen` sessions open, please close some and try again")
				}
				exitCh <- struct{}{}
				return
			}
...
func Init(ctx context.Context, cfg *Config) (*Proxy, error) {
	...
	if cfg.UseConfiguredWebhooks && len(cfg.ForwardURL) > 0 {
		if strings.HasPrefix(cfg.ForwardURL, "/") {
			return nil, errorcategory.New(errorcategory.API, "forward_to cannot be a relative path when loading webhook endpoints from the API")
		}
		if strings.HasPrefix(cfg.ForwardConnectURL, "/") {
			return nil, errorcategory.New(errorcategory.API, "forward_connect_to cannot be a relative path when loading webhook endpoints from the API")
		}
	} else if cfg.UseConfiguredWebhooks && len(cfg.ForwardURL) == 0 {
		return nil, errorcategory.New(errorcategory.API, "load_from_webhooks_api requires a location to forward to with forward_to")
	}
	...
	if cfg.UseConfiguredWebhooks {
		...
		if len(endpoints.Data) == 0 {
			return nil, errorcategory.New(errorcategory.API, "you have not defined any webhook endpoints on your account, go to the Stripe Dashboard to add some: https://dashboard.stripe.com/test/webhooks")
		}
		var err error
...
func ExtractRequestData(data interface{}) (StripeRequest, error) {
	...
	return StripeRequest{}, errorcategory.New(errorcategory.API, "received malformed event from Stripe")
}
...
func buildForwardURL(forwardURL string, destination *url.URL) (string, error) {
	...
	if err != nil {
		return "", errorcategory.Errorf(errorcategory.API, "provided forward url cannot be parsed: %s", forwardURL)
	}
