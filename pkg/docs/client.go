...
// WithStripeVersion sets the Stripe-Version header sent on authenticated API requests.
func WithStripeVersion(v string) ClientOption {
	return func(c *Client) { c.stripeVersion = v }
}
...
func (c *Client) fetchPageViaAPI(ctx context.Context, ref *url.URL) (Page, error) {
	...
	if c.stripeVersion != "" {
		req.Header.Set("Stripe-Version", c.stripeVersion)
	}
