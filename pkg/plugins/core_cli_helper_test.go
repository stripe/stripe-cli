...
func TestListAuthorizedAccountsReturnsConfigTypeMismatchError(t *testing.T) {
	coreCLIHelper := NewCoreCLIHelper(context.Background(), nil, afero.NewMemMapFs(), "", "", "")
	accounts, activeAccountID, _, err := coreCLIHelper.ListAuthorizedAccounts()
	require.Error(t, err)
	require.Empty(t, accounts)
	require.Empty(t, activeAccountID)
}

func TestListAuthorizedAccountsSuccess(t *testing.T) {
	original := loginListAuthorizedAccountsForActiveSession
	t.Cleanup(func() { loginListAuthorizedAccountsForActiveSession = original })

	want := []config.AuthorizedAccount{
		{ID: "acct_123", Name: "Test Co", Modes: []string{"test"}},
		{ID: "acct_456", Name: "Live Co", Modes: []string{"live", "test"}},
	}
	loginListAuthorizedAccountsForActiveSession = func(ctx context.Context, accessBaseURL string, cfg *config.Config) (*login.AuthorizedAccountsResult, error) {
		return &login.AuthorizedAccountsResult{
			Accounts:        want,
			ActiveAccountID: "acct_456",
			ActiveLivemode:  true,
		}, nil
	}

	coreCLIHelper := NewCoreCLIHelper(context.Background(), &config.Config{}, afero.NewMemMapFs(), "", "", "")
	accounts, activeAccountID, activeLivemode, err := coreCLIHelper.ListAuthorizedAccounts()
	require.NoError(t, err)
	require.Equal(t, want, accounts)
	require.Equal(t, "acct_456", activeAccountID)
	require.True(t, activeLivemode)
}

func TestListAuthorizedAccountsPropagatesError(t *testing.T) {
	original := loginListAuthorizedAccountsForActiveSession
	t.Cleanup(func() { loginListAuthorizedAccountsForActiveSession = original })
