...
import (
	...
	"github.com/stripe/stripe-cli/pkg/genresources/utils"
...
func GetTemplateData(stripeAPI *spec.Spec) (*TemplateData, error) {
	data := &TemplateData{
			...
			if media, ok := requestContent["application/x-www-form-urlencoded"]; ok {
				for propName, schema := range media.Schema.Properties {
					...
					if schema.Type == "object" {
						denormalizedProps := utils.DenormalizeObject(propName, schema)
						for prop, propType := range denormalizedProps {
							properties[prop] = propType
						}

					} else {
						scalarType := utils.GetType(schema)
