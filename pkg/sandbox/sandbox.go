...
var (
	ErrUnsupportedAlgorithm = errorcategory.New(errorcategory.UserInput, "unsupported algorithm: only SHA-256 is supported")
...
func (c *Client) GetChallenge(ctx context.Context, email string) (*ChallengeResponse, error) {
	...
	if challenge.Algorithm == "" || challenge.Challenge == "" {
		return nil, errorcategory.Errorf(errorcategory.API, "invalid challenge response: missing algorithm or challenge")
	}
