...
var (
	...
	ErrActivateReaderFailed = errorcategory.New(errorcategory.Network, "couldn't communicate with the Reader. Please make sure your reader is online and on the same network as your device.\nSee our troubleshooting docs here: https://stripe.com/docs/terminal/readers/verifone-p400#troubleshooting")
	// ErrRegisterReaderFailed is for when adding the reader via the Stripe API could not be completed (likely bad reg code)
	ErrRegisterReaderFailed = errorcategory.New(errorcategory.UserInput, "could not register the Reader due to an invalid reader code")
	// ErrReaderSelectionFailed is for when the user quit the CLI at the reader choice prompt
	ErrReaderSelectionFailed = errorcategory.New(errorcategory.UserInput, "reader choice failed, no selection made")
	...
	ErrNoReadersRegistered = errorcategory.New(errorcategory.UserInput, "no readers currently registered")
	...
	ErrSetReaderDisplayFailed = errorcategory.New(errorcategory.Network, "could not set the Reader's display")
	...
	ErrClearReaderDisplayFailed = errorcategory.New(errorcategory.Network, "could not clear the Reader's display")
	// ErrCollectPaymentFailed is for when the Rabbit call for the reader to go into collect payment state failed
	ErrCollectPaymentFailed = errorcategory.New(errorcategory.Network, "could not collect payment method")
	// ErrCollectPaymentTimeout is for when the user didn't boop the card on the reader in a reasonable time
	ErrCollectPaymentTimeout = errorcategory.New(errorcategory.Network, "timed out waiting for payment to be presented")
	// ErrConfirmPaymentFailed is for when the Rabbit call to confirm the payment method collected has failed
	ErrConfirmPaymentFailed = errorcategory.New(errorcategory.Network, "could not confirm the payment")
	...
	ErrQueryPaymentFailed = errorcategory.New(errorcategory.Network, "could not query the payment")
	...
	ErrDNSFailed = errorcategory.New(errorcategory.Network, "couldn't find your reader on the network. We think it's probably a DNS issue.\n See our troubleshooting docs here: https://stripe.com/docs/terminal/readers/verifone-p400#troubleshooting")
	...
	ErrRabbitRequestCreationFailed = errorcategory.New(errorcategory.Internal, "could not prepare request for Reader")
	...
	ErrStripeForbiddenResponse = errorcategory.New(errorcategory.Auth, "it seems that your Stripe API Key is either restricted or out of date. Are you using a valid Stripe Secret Key with the --api-key global flag for this command?")
