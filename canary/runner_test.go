...
func TestAPICreateDeleteCustomer(t *testing.T) {
	...
	// Delete the customer to clean up
	deleteResult, err := runner.Run("delete", "/v1/customers/"+customer.ID)
	if err != nil {
...
func TestAPIProductsCreate(t *testing.T) {
	...
	if err != nil {
		...
		t.Logf("Warning: Archive returned non-zero exit code: %d", archiveResult.ExitCode)
	}
}
